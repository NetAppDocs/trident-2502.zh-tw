...
NOTE: 當您使用 Kopia 作為資料移動器還原備份時，您可以選擇在 CR 中或使用 CLI 指定註解來控制 Kopia 使用的暫存的行為。請參閱 https://kopia.io/docs/getting-started/["Kopia 文件"^]有關您可以配置的選項的詳細資訊。使用 `tridentctl-protect create --help`有關使用Trident Protect CLI 指定註釋的更多信息，請參閱命令。
